---
status: confirmed
attribution: user-confirmed
updated: 2026-09-05
sources:
  - SRC-2026-09-04-tender-systems-identity-review
  - SRC-2026-09-05-social-account-strategy
---

# DEC-005 — Public-Facing Language

## Status

accepted (2026-09-05)

## Decision

Public copy states what a system does; it never explains, interprets, or labels the work.

- Public text does not describe TENDER SYSTEMS or its systems as an "art project," and does not declare the institution to be a "fictional institution." The user supplied these as established policy inside a pasted instruction block:

  > "지나치게 세계관을 설명하거나 'fictional institution'이라고 스스로 밝히지 않는다."

  and the prohibition on defining TENDER SYSTEMS as an "art project" (listed among actions not to take: `TENDER SYSTEMS를 "art project"라고 직접 정의`).

- Public copy does not carry personal attribution; see [[DEC-003-public-identity]].

- Copy that explains the work's meaning is rejected. This rule was enforced by the user through repeated rejection of drafted bios rather than stated as an abstract principle, and the user wrote or shortened the final wording in each case:

  > "human experience 가 너무 대놓고 작품을 설명하는 것 같아서 별로네. 다시 한번 생각해봐."

  > "근데 너무 길다."

- Systems speak in their own operating register rather than as sub-brands announcing a parent. The assistant proposed the specific anti-pattern of bios reading "An art project by TENDER SYSTEMS," and the user did not contradict it; this element is `llm-proposed`.

## Rationale

The work is encountered through the surface it presents; copy that explains the surface removes the encounter. An institution reports what it operates.

## Consequences

- Public bios, profile text, repository descriptions, and README copy state operations, not interpretation.
- The per-system public copy strings themselves are maintained in each system's Lab repository, not here.
- Internal working vocabulary may describe the institution more directly than public copy does — that boundary is open; see [[Q-002-internal-versus-public-language]].

## Related

- [[DEC-003-public-identity]]
- [[DEC-004-institutional-voice]]
- [[public-surfaces]]
- [[Q-002-internal-versus-public-language]]

## Sources

- [[SRC-2026-09-04-tender-systems-identity-review]] — raw/conversations/2026-09-04-tender-systems-identity-review.md
- [[SRC-2026-09-05-social-account-strategy]] — raw/conversations/2026-09-05-social-account-strategy.md

In the identity conversation these rules appear inside a long instruction block the user pasted from work done elsewhere, so they are recorded there as already-established policy rather than newly decided. The social-account conversation evidences the rule through the user's rejections of specific drafts rather than through a general statement.
