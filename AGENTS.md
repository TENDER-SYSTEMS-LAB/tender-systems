...
This is the institutional Wiki and the canonical convention for all TENDER SYSTEMS Labs. Local Lab rules remain self-contained; do not open sibling repositories for routine work. Read `wiki/registry.md` only for system identifiers, status, or cross-system relationships. Detailed project knowledge stays in its Lab; use `wiki/decisions/DEC-002-decision-placement.md` when placement is unclear.
